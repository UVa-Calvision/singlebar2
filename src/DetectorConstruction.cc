...
DetectorConstruction::DetectorConstruction(ConfigEnvironment& e)
  : env(e)
{
  //---------------------------------------
  //------------- Parameters --------------
...
#define USE_CALOPV
  ...
  const int layerOption = env.file().read<int>("detector_layers");
  ...
  const unsigned int rearCrystalLayerIndex = env.useSipmR() ? 4 : 3;
